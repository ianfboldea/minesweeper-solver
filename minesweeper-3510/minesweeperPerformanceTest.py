    ...
    def parseAIAlgo(self, userCommand):
        if type(userCommand) is not tuple:
            ...
            if set(self.bombLocations) == set(userAnswer):
                self.outcome = 1
                print("CORRECT BOMB LIST! You performed {} digs.".format(self.numDigs))
                dataToWrite = "({}*{}, {}, {}, {})".format(self.numRows, self.numCols, self.numBombs, self.numDigs, self.time)
                f = open("data2.py", "a")
                f.write("trials.append({})\n".format(dataToWrite))
                f.close()
            else:
                self.outcome = -1
                print("WRONG BOMB LIST. expected: {}, received: {}. You performed {} digs.".format(self.bombLocations, userAnswer, self.numDigs))
